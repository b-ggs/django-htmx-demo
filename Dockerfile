...
RUN useradd --create-home htmx_demo

# Set up project directory
ENV APP_DIR=/app
RUN mkdir -p "$APP_DIR" \
  && chown -R htmx_demo "$APP_DIR"

# Set up virtualenv
ENV VIRTUAL_ENV=/venv
ENV PATH=$VIRTUAL_ENV/bin:$PATH
RUN mkdir -p "$VIRTUAL_ENV" \
  && chown -R htmx_demo:htmx_demo "$VIRTUAL_ENV"
...
RUN curl -sSL https://install.python-poetry.org | python3 - \
  && chown -R htmx_demo:htmx_demo "$POETRY_HOME"
...
COPY --chown=htmx_demo pyproject.toml poetry.lock ./
...
CMD ["gunicorn", "htmx_demo.wsgi:application"]
...
COPY --chown=htmx_demo:htmx_demo . .
...
RUN echo "alias djtest='./manage.py test --settings=htmx_demo.settings.test -v=2'" >> $HOME/.bash_aliases
RUN echo "alias djtestkeepdb='./manage.py test --settings=htmx_demo.settings.test -v=2 --keepdb'" >> $HOME/.bash_aliases
